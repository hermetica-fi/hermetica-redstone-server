...
// at `scripts/sign-stacksjs.ts` for an example at how to use this library. It should
// work for both the server and the client side.

import { keccak_256 } from '@noble/hashes/sha3';
import { bufferCV, listCV, tupleCV, uintCV } from "@stacks/transactions";
import { hexToBytes as hexToBytesMS, concatBytes } from "@stacks/common";
import { compressPublicKey, serializePublicKey } from "@stacks/transactions";
 ...
 * @returns
 */
export function liteDataHash(liteByteString) {
 ...
 * @returns
 */
export function liteDataHashPersonalSign(liteDataHash) {
	return keccak_256(concatBytes(ethPersonalSignPrefix, liteDataHash));
}
 ...
 * @returns Uint8Array
 */
export function compressRedstonePubkey(pubKey) {
	if (pubKey.length === 33)
		return pubKey;
 ...
 * @returns Uint8Array
 */
export function liteSignatureToStacksSignature(liteSignature) {
 ...
 * @returns Object
 */
export function pricePackageToCV(pricePackage) {
	return {
		timestamp: uintCV(pricePackage.timestamp),
		prices: listCV(
			pricePackage.prices.map((entry) => tupleCV({
 ...
 * @returns BufferCV
 */
export function liteSignatureToBufferCV(liteSignature) {
	if (typeof liteSignature === 'string')
		liteSignature = hexToBytes(liteSignature);
	if (liteSignature.byteLength !== 65)
		throw new Error(`Invalid liteSignature, expected 65 bytes got ${liteSignature.byteLength}`);
